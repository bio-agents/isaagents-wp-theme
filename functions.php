...
$agents = new JW_Post_Type('Agents', array(
   'supports' => array('title', 'revisions'),
   'labels' => array(
               'name'      => 'Agent',
               'all_items' => 'Agents',
...
$agents->add_meta_box('Agent Info', array(
	'description' => 'textarea',
	'version' => 'text',
	'apple_download' => 'text',
	'windows_download' => 'text',
	'linux_download' => 'text',
	'agent type' => array('select', array('Analysis','Annotation', 'API', 'Configuration', 'Conversion', 'Ontology', 'Parsers', 'Validation', 'Web')),
	'source' => 'text',
	'docs' => 'text',
	'issues' => 'text',
	'videos' => 'text',
...
function people_function($atts) {
	...
	if ( $the_query->have_posts() ) {
    	...
    	while ( $the_query->have_posts() ) {
    		...
    		$str .='<div id="'.get_the_ID().'" class="isaagents-person">';
...
function output_filter() {
	$agent_types = array('Analysis', 'Annotation', 'API', 'Configuration', 'Conversion', 'Ontology', 'Parsers', 'Validation', 'Web');
	$filter_str = '<ul id="filter-items" class="splitter"><li><ul>';
	$filter_str .= '<li class="selected"><span data-value="all">All</span></li>';
	foreach ($agent_types as $key => $val) {
...
function agents_function($attrs) {
	$the_query = new WP_Query(array('post_type' => 'Agents', 'posts_per_page' => -1, 'orderby' => 'id', 'order' =>'ASC'));
    $str = "";
    ...
    if ( $the_query->have_posts() ) {
    	...
    	$str .='<ul id="agent-list" class="service-list">';
        while ( $the_query->have_posts() ) {
        	$the_query->the_post();

        	$str .='<li id="'.get_the_ID().'" class="agent_block '.get_post_meta( get_the_ID(), 'agent_info_agent_type', true ).'" style="margin-left:0px; margin-right: 4px; margin-bottom: 10px">';
        	$str .= '<div align="center" id="'.get_the_title().'" class="services">';
        	$str .= '<img src="'.get_post_meta( get_the_ID(),'agent_info_logo', true ).'" alt="'.get_the_title().'" style="float:none"><div class="cf"></div>';
        	$str .= '<div class="descriptionBlock">'.get_post_meta( get_the_ID(),'agent_info_description', true ).'</div>';

        	$str .= '<div class="download">';
        	foreach ($supported_os as $key => $val) {
				$meta_key = 'agent_info_'.$val.'_download';
